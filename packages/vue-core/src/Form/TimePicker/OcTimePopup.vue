...
const scrollIntoActiveDate = () => {
  ...
  const activeHourElement = hourList.value.querySelector(`.w-7:nth-child(${hour.value})`)
...
const scrollToNearestHour = () => {
  ...
  hours.forEach((h, i) => {
    if (Math.abs(h.getBoundingClientRect().top - box.getBoundingClientRect().top) < 12) {
      hour.value = i + 1
      scrollIntoActiveDate()
    }
  })
...
const scrollToNearestMinute = () => {
  ...
  minutesArray.forEach((m, i) => {
    if (Math.abs(m.getBoundingClientRect().top - box.getBoundingClientRect().top) < 12) {
      minutes.value = i
      scrollIntoActiveDate()
    }
  })
...
const scrollToNearestPeriod = () => {
  ...
  periods.forEach((p, i) => {
    if (Math.abs(p.getBoundingClientRect().top - box.getBoundingClientRect().top) < 12) {
...
<template>
  <div class="overflow-hidden flex items-center relative rounded shadow-normal w-fit px-3">
    <div ref="triggerBox" class="bg-oc-accent-1-50-tr border-y absolute h-7 left-0 right-0" />
    ...
    >
      ...
      >
        <li
          ...
          :class="period === (i === 1 ? 'AM' : 'PM') ? 'opacity-100' : 'opacity-50'"
