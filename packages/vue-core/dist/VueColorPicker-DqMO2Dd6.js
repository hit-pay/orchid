import { ref as k, watch as _, openBlock as $, createBlock as Y, unref as U, withKeys as Ue, createElementBlock as C, createVNode as D, withCtx as K, createTextVNode as Z, inject as se, createElementVNode as w, normalizeClass as J, normalizeStyle as W, onMounted as Ne, createCommentVNode as H, computed as lt, reactive as Le, provide as oe, nextTick as at, Fragment as Re, renderList as Be, withDirectives as nt, vShow as rt } from "vue";
import "dayjs";
...
import { _ as ie } from "./OcInput-BhmeVr1A.js";
...
import { _ as ot } from "./OcOption-B8-9zkBz.js";
import { _ as st } from "./OcIcon-CJgBICxh.js";
import { S as fe } from "./OcSlider-Bsi-ckbP.js";
...
const pt = {
    ...
    const c = i, f = V, s = k({
      ...
      if (c.inputType == "HSL" && (x = 100, S == "r" && (x = 360)), !isNaN(parseInt(y))) {
        if (S == "r") {
          ...
          let E = parseInt(y);
          E <= x && (s.value.b = E, c.inputType == "RGB" ? M("b") : j()), (P = u.value) == null || P.focus();
        }
      }
    }, M = (S) => {
      f("update-rgb-value", {
  ...
  setup(i, { emit: V }) {
    ...
    return (b, u) => ($(), C("div", wt, [
      i.gradientType == "linear" && c.value ? ($(), Y(U(fe), {
        key: 0,
        label: "Angle",
        "model-value": i.angle,
        "min-limit": 0,
...
}, Et = { key: 2 }, Xt = { key: 0 }, Mt = { class: "ck-cp-local-color-conatiner" }, Ut = ["onClick"], Nt = { class: "flex items-center gap-2" }, Ot = { id: "ck-cp-target-background" }, Gt = { class: "flex gap-3" }, Yt = {
  key: 0,
  ...
  setup(i, { emit: V }) {
    const c = i, f = k(c.variant == "gradient" ? "gradient" : "solid"), s = k(null), I = V, b = k(c.modelValue), u = (t) => {
      b.value = t, P.value && I("update:modelValue", t);
    }, a = k([
    ...
    const j = lt(() => c.variant !== "gradient" ? M.value.filter((t) => !t.includes("linear") && !t.includes("radial")) : M.value), S = k(!1), y = k("linear"), x = Le({
      ...
      percentageX: 50,
      percentageY: 50
    }), N = k(""), B = k(c.inputType), P = k(!1), E = k(), ue = k(), z = k(), O = k(), h = k(), L = Le({
      h: 0,
      s: 0,
      l: 0
    });
    ...
    const Oe = (t) => {
      ...
      if (t) {
        ...
        let n = e, r = l;
        Number.isNaN(r) && (r = n);
        let o = { x: 0, y: 0 }, p = O.value.offsetWidth - h.value.offsetWidth, v = O.value.offsetHeight - h.value.offsetHeight;
        const [g, d] = 2 * n - 1 < 0 ? [
          p * 2 * r / (1 + r),
          v * (1 - n * (1 + r))
        ] : [
    ...
    const Ie = (t) => {
      ...
      if ((n = l == null ? void 0 : l.id) != null && n.includes("clr-gb-")) {
        ...
        X = l;
        const p = a.value.find((v) => v.select == !0);
        if (p && p.id != o) {
          const v = (r = s.value) == null ? void 0 : r.querySelector(".gradient-handle.select");
          v == null || v.classList.remove("select"), l.classList.add("select");
          const g = a.value.findIndex((d) => d.select == !0);
          for (let d = 0; d < a.value.length; d++) {
            const m = a.value[d];
            if (m.id == o) {
              g != -1 && (a.value[g].select = !1), a.value[d].select = !0, A.value = m.a, re(m.r, m.g, m.b, m.hue, !0), q(), Q();
              break;
            }
          }
        }
      ...
      if (r < 0 ? r = 0 : r > n && (r = n), X) {
        X.style.left = `${r}px`;
        const v = parseFloat((r / ((l == null ? void 0 : l.width) - (e == null ? void 0 : e.width)) * 100).toFixed(0)), g = a.value.find(
      ...
      a.value[n].select = !1;
      const p = (v = s.value) == null ? void 0 : v.querySelector(".gradient-handle.select");
      ...
      if (t.r == 255 && t.g == 255 && t.b == 255)
        return !0;
      {
        const e = z.value.getContext("2d", { willReadFrequently: !0 }), l = z.value.width, n = z.value.height, r = e.getImageData(0, 0, l, n).data, o = t.r, p = t.g, v = t.b;
        for (let g = 0; g < n; g++)
          for (let d = 0; d < l; d++) {
            const m = (g * l + d) * 4, G = r[m], et = r[m + 1], tt = r[m + 2];
            if (G === o && et === p && tt === v)
              return !0;
          }
        return !1;
      ...
      setTimeout(() => {
        if (Ve({ r: t, g: e, b: l })) {
          ...
          let o = ne();
          o && (h.value.style.left = `${o.x}px`, h.value.style.top = `${o.y}px`), F.value = n, ge(r);
        }
      }, 0);
    }, me = () => {
      ...
      if (f.value == "gradient") {
        ...
        e.id = `clr-gb-${t.id}`, e.classList.add("gradient-handle"), e.style.left = Pe(t.percent);
      ...
      if (f.value == "gradient") {
        ...
        if (y.value == "linear") {
          ...
          for (let r = 0; r < a.value.length; r++) {
            const { r: o, g: p, b: v, a: g, percent: d } = a.value[r];
            a.value.length - 1 == r ? (e = e + `rgba(${o},${p},${v},${g / 100}) ${d}%)`, l = l + `rgba(${o},${p},${v},${g / 100}) ${d}%)`) : (e = e + `rgba(${o},${p},${v},${g / 100}) ${d}%, `, l = l + `rgba(${o},${p},${v},${g / 100}) ${d}%, `);
          }
        } else {
        ...
        n && (n.style.backgroundImage = l, P.value && u(n.style.backgroundImage));
      } else {
      ...
      t == null || t.classList.add("active"), new EyeDropper().open().then((n) => {
        const { sRGBHex: r } = n, o = a.value.find((v) => v.select == !0), p = be(r);
        if (p) {
          const v = ce(p.r, p.g, p.b);
          if (o && (o.hue = v, o.r = p.r, o.g = p.g, o.b = p.b), Ve(p)) {
            const g = ne();
            g && (h.value.style.left = `${g.x}px`, h.value.style.top = `${g.y}px`);
          } else {
            F.value = v;
            const { rgb: g } = De(v, 100, 50);
            Se(g);
            const d = ne();
          ...
          if (r) {
            ...
            for (let v = 0; v < p.length; v++) {
              ...
              if (g.includes("deg"))
                ...
                let m = g.trim().replace(/rgba|\(|\)|%/g, "").replace(/,/g, " ").split(" ");
                m = m.filter((G) => G.trim() !== ""), d.r = parseInt(m[0]), d.g = parseInt(m[1]), d.b = parseInt(m[2]), d.a = parseFloat(m[3]) * 100, d.percent = parseInt(m[4]), l.push(d);
              } else if (g.includes("rgb")) {
                let m = g.trim().replace(/rgb|\(|\)|%/g, "").replace(/,/g, " ").split(" ");
                m = m.filter((G) => G.trim() !== ""), d.r = parseInt(m[0]), d.g = parseInt(m[1]), d.b = parseInt(m[2]), d.percent = parseInt(m[3]), l.push(d);
              }
            }
          }
        } else {
          ...
          if (r) {
            ...
            for (let v = 0; v < p.length; v++) {
              ...
              if (g.includes("circle at")) {
                let m = g.replace("circle at ", "").replace(" ", "").replace(/%/g, " ").trim().split(" ");
                x.percentageX = parseInt(m[0]), x.percentageY = parseInt(m[1]);
              } else if (g.includes("rgba")) {
                let m = g.replace(/rgba|\(|\)|%/g, "").replace(/,/g, " ").split(" ");
                m = m.filter((G) => G.trim() !== ""), d.r = parseInt(m[0]), d.g = parseInt(m[1]), d.b = parseInt(m[2]), d.a = parseFloat(m[3]) * 100, d.percent = parseInt(m[4]), l.push(d);
              } else if (g.includes("rgb")) {
                let m = g.replace(/rgb|\(|\)|%/g, "").replace(/,/g, " ").split(" ");
                m = m.filter((G) => G.trim() !== ""), d.r = parseInt(m[0]), d.g = parseInt(m[1]), d.b = parseInt(m[2]), d.percent = parseInt(m[3]), l.push(d);
              }
            }
          }
    ...
    const _e = (t) => {
      a.value.forEach((e) => {
        ...
        var e;
        T = (e = s.value) == null ? void 0 : e.querySelector(".gradient-bar"), We(t), me();
    ...
    return Ne(() => {
      var t;
      f.value = c.modelValue.includes("linear") || c.modelValue.includes("radial") ? "gradient" : "solid", f.value == "gradient" && (T = (t = s.value) == null ? void 0 : t.querySelector(".gradient-bar")), ve(c.modelValue), de(B.value), at(() => {
        P.value = !0;
      });
    }), (t, e) => ($(), C("div", {
      ...
      w("div", Gt, [
        ...
        P.value && i.showInputSet ? ($(), C("div", Yt, [
          B.value == "HEX" ? ($(), Y(pt, {
            key: 0,
            modelValue: N.value,
            "onUpdate:modelValue": [
      ...
      M.value.length > 0 && i.showColorList ? ($(), C("div", Pt, [
