...
export const useUploadFileProgress = (
  ...
  const onChangeFile = (event, singleFileUpload = false) => {
    ...
    for (let i = 0; i < uploadFiles.length; i++) {
      ...
      emit('update:modelValue', !singleFileUpload ? currentFiles.value : currentFiles.value[0])
      if (file) {
  ...
  const addListeners = (reader, index) => {
    reader.addEventListener('loadstart', (e) => handleEventFile(e, reader, index))
    reader.addEventListener('load', (e) => handleEventFile(e, reader, index))
    reader.addEventListener('loadend', (e) => handleEventFile(e, reader, index))
    reader.addEventListener('progress', (e) => handleEventFile(e, reader, index))
  ...
  const onDeleteFile = (index) => {
    ...
    emit('update:modelValue', currentFiles.value.length > 0 ? currentFiles.value : null)
  }
  return {
    isErrorMaxSize,
    currentFiles,
